...
void main() {
    ...
    if (r >= 1.0) {
        ...
        float planetCoverage, sunCoverage;
        float planetSunOverlap = fractionalCoverage(normal,
                vec3(u_planet_pos, 0.0), u_planet_radius, vec3(u_sun_pos, 0.0), u_sun_radius,
                planetCoverage, sunCoverage);
        
        float b = dot(normal, lightDir)*(sunCoverage - planetSunOverlap);
